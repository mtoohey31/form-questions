...
<MaterialApp {theme}>
    <AppBar class="yellow darken-2">
        <span slot="title">Form Questions</span>
        ...
        {/if}
        <Tooltip bottom>
            <Button class="ma-2" on:click={() => (spreadsheetSelection = true)}>
            ...
            <Col>
                {#if !currentTransition}
                    <div
                        ...
                        transition:fade={{ duration: 200 }}>
                        <Card>
                            <Chip label class="mt-4 ml-4 mr-4 yellow darken-2">
    ...
    {:else}
        <Alert class="error-color mt-4">
            ...
            {:else if typeof spreadsheetID === 'undefined' || spreadsheetID.length !== 44}
                <div class="text-body-1">• Enter a valid spreadsheet ID</div>
            {/if}
        </Alert>
        <Footer class="justify-center pa-2" absolute>
            <ProgressLinear
